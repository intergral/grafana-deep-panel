...
import {VariableValue} from "./VariableGroup";
    ...
    return (
        <div className={border ? styles.itemView : ''}>
            <div className={styles.itemKeyLine}>
                <span className={styles.itemKey}>{itemKey}{keyDecoration}</span>
            </div>
            <div className={styles.itemValueLine}>
                <span className={styles.itemValue}>{itemValue}{valueDecoration}</span>
            </div>
        </div>
    )
}
...
export function TracepointView({tracepoint}: { tracepoint: Tracepoint }) {
                ...
                <ul className={styles.clearList}>
                    {
                        watches.map((watch, index) => {
                            return <li key={index}><span>{watch}</span></li>
                        })
                    }
                </ul>
            </VerticalGroup>
                ...
                <ul className={styles.clearList}>
                    {
                        filteredArgs.map((key, index) => {
                            ...
                            return <li key={index}><ItemView itemKey={key} itemValue={value} border={false}/></li>
                        })
                    }
                </ul>
            </VerticalGroup>
    ...
    return (
        <div>
            ...
            <ItemView itemKey='Line' itemValue={tracepoint.line_number.toString(10)}/>
            <ItemView itemKey='Fire Count' itemValue={fireCount}
                      valueDecoration={fireCount === '-1' && <span className={styles.txtLight}> (infinite)</span>}/>
            <ItemView itemKey='Fire Period' itemValue={tracepoint.args?.fire_period ?? '1000'}
                      valueDecoration={<span className={styles.txtLight}> ms</span>}/>
            <ItemView itemKey='Watches' itemValue='' valueDecoration={displayWatches()}/>
            <ItemView itemKey='Args' keyDecoration={<span className={styles.txtLight}> (additional)</span>} itemValue=''
        ...
        return (
            <VerticalGroup align="center" justify="flex-start">
                <Alert title="No watch results" severity='info'>
                    <div>To learn more about watches <a href="#" className={styles.link}>read the docs</a>.</div>
                </Alert>
            </VerticalGroup>
        )
    }
            ...
            <VerticalGroup>
                {
                    results.map((val, index) => {
                        return (
                            <div key={index}>
                                {val.Result.GoodResult && <div><VariableValue variableID={val.Result.GoodResult}
                                                                              lookup={lookupFunc}
                                                                              depth={0}/></div>}
                                {val.Result.ErrorResult && <div><span>{val.Result.ErrorResult}</span></div>}
                            </div>
                        )
                    })
                }
        ...
        return (
            <VerticalGroup align="center" justify="flex-start">
                <Alert title={`No ${name} values`} severity='info'>
                    {link &&
                        <div>To learn more about {name} <a href="#" className={styles.link}>read the docs</a>.</div>}
                </Alert>
            </VerticalGroup>
        )
    }
        ...
        <div>
            <VerticalGroup>
                <ul className={styles.fullWidth}>
                    {keys.map((key, index) => {
                        return <li key={index}><ItemView itemKey={key} itemValue={attrs[key]}/></li>
                    })}
                </ul>
            </VerticalGroup>
        </div>
...
export function SnapshotMetaGroup({tracepoint, lookup, watchResults, height, resource, attributes}: Props) {
    ...
    return (
        <div className={cx(css`height: ${height}px`, styles.snapMeta)}>
            <VerticalGroup>
                <TabsBar>
                    {state.map((tab, index) => {
                        return (
                            <Tab
                                ...
                                label={tab.label}
                                active={tab.active}
                                onChangeTab={() => updateState(state.map((tab, idx) => ({
                                    ...tab,
                                    active: idx === index
                                })))}
                            />
                ...
                <TabContent className={cx(styles.scroll, css`width: 100%;
                  height: ${height - 47}px`)}>
